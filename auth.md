...
Sesssions
http was the backbone for all communication on the web. but was/is stateless. was ideal for early web because pages were static. 
websites transitioned to dynamic content, statelessness of http became a bottleneck. stateful interactions became a need.

## Sesssions

a way to establish temporary server side context for each user. 

how did it work? 

### session creation

when the user logged in the server created a unique session id and it stored it alongside the data in a persistent store (database or redis)

### session 
session id is sent to the client as a cookie. all the requests the client made after that request included that cookie. 
...
hybrid approach, raises some questions again. 
...
user sends the username and pw ->  server validates it and generates a signed JWT token with a secret key. The server has it's own secret key using which it can sign and verify JWTs. -> sends back to client -> the client can send the JWT back to the server in the header called authorization. -> server extracts the token and tries to vierify using the secret key and then finds out user id. 
